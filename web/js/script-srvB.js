...
// ===== CPU =====
fetch('./data/cpu_info.json?t=' + Date.now())
  .then(r => r.json())
  .then(cpu => {
...
fetch('../data/connexions.json')
  .then(r => r.json())
  .then(data => {
    const div = document.getElementById('connexions-info');

    if (!data.Connexions_Establertes || data.Connexions_Establertes.length === 0) {
    ...
    data.Connexions_Establertes.forEach(c => {
      html += `
        <tr>
          <td>${c.Protocol}</td>
          <td><span class="badge badge-active">${c.State}</span></td>
          <td>${c.Local_IP}</td>
          <td>${c.Local_Port}</td>
          <td>${c.Remote_IP}</td>
          <td>${c.Remote_Port}</td>
  ...
  .catch(err => {
    console.error('Error connexions.json', err);
    document.getElementById('connexions-info').innerHTML =
      '<p class="placeholder">Connections data not available</p>';
  });

// ===== SERVEIS =====
fetch('../data/services_status.json')
  .then(r => r.json())
  .then(data => {
    const div = document.getElementById('serveis-info');

    if (!data.Estat_de_Serveis || data.Estat_de_Serveis.length === 0) {
      div.innerHTML = '<p class="placeholder">No services found</p>';
      return;
    }
    ...
    data.Estat_de_Serveis.forEach(s => {
      const uptime = formatUptime(s.Uptime_segons);
      const isActive = s.Estat === 'active';
      html += `
        <tr>
          <td>${s.Nom_servei}</td>
          <td><span class="badge ${isActive ? 'badge-active' : 'badge-inactive'}">${s.Estat}</span></td>
          <td>${uptime}</td>
          <td>${s.Port_escolta}</td>
        </tr>
      `;
    });
  ...
  .catch(err => {
    console.error('Error serveis.json', err);
    document.getElementById('serveis-info').innerHTML =
      '<p class="placeholder">Services data not available</p>';
  });
